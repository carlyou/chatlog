...
export function getStableRootId(root: Element): string {
...
function digestTextSample(text: string): string {
  ...
  const tail =
    len > TEXT_DIGEST_SAMPLE_SIZE ? text.slice(-TEXT_DIGEST_SAMPLE_SIZE) : '';
...
export function extractStructuredContent(element: Element): StructuredContent {
  ...
  element
    .querySelectorAll('h1, h2, h3, h4, h5, h6, p, ul, ol, pre, hr, img, table')
    .forEach((el) => {
      ...
      if (/^h[1-6]$/.test(tag)) {
        ...
        for (const d of el.querySelectorAll(
          'p, h1, h2, h3, h4, h5, h6, ul, ol, pre, hr, img',
        ))
          visited.add(d);
        const items: RichText[] = [];
        ...
        if (result.type === 'broken') {
          ...
          if (result.code) {
            ...
            if (result.markdown) {
              const { fragment, blocks: parsed } = parseMarkdownToElements(
                result.markdown,
                classMap,
                codeBlockWrapper,
              );
...
export function extractBranchInfo(container: Element): BranchInfo | undefined {
...
/** Compute a signature string for a message root, given a pre-determined role. */
export function computeBaseSignature(role: string, root: Element): string {
...
// --- Adapter-delegating public API (preserves signatures for consumers) ---

export function getMessageRootSelector(platform: Platform): string | null {
  if (!platform) return null;
  const adapter = getAdapter(platform);
  return adapter?.messageRootSelector ?? null;
}

export function getMessageRoots(
  platform: Platform,
  container: Element,
): Element[] {
...
export function getMessageRootForNode(
  platform: Platform,
  node: Node,
): Element | null {
  const selector = getMessageRootSelector(platform);
...
export function computeMessageRootSignature(
  platform: Platform,
  root: Element,
): string {
  if (!platform) return '';
  const adapter = getAdapter(platform);
  if (!adapter) return '';
  return adapter.computeSignature(root);
}

export function parseMessageRoot(
  platform: Platform,
  root: Element,
): Message | null {
