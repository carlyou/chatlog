...
type TimingKey = 'parseMessagesMs' | 'refreshMs';
...
function maybeReport() {
  ...
  reportTimer = window.setInterval(() => {
    ...
    console.log(
      ...
      `fullParses=${counters.fullParses} parse(avg/p95/max)=${mean(parse).toFixed(1)}/${p95(parse).toFixed(1)}/${max(parse).toFixed(1)}ms ` +
